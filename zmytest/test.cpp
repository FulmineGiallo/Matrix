...
namespace lasd
{
  ...
  void popolaMatrix(lasd::MatrixCSR<Data>& mat, const unsigned long& righe, const unsigned long& colonne, const unsigned long& numValori)
  {
    ...
    uniform_int_distribution <unsigned long> distRow(0,righe - 1);
    uniform_int_distribution <unsigned long> distCol(0,colonne - 1);
    uniform_int_distribution <int> dist(-255,255);
    unsigned long r, c;
    mat.RowResize(righe);
    mat.ColumnResize(colonne);
    for(unsigned long i = 0; i < numValori; i++){
      r = distRow(gen);
      c = distCol(gen);
      mat(r,c) = dist(gen);
    }
}
  void popolaMatrixString(lasd::MatrixCSR<string>& mat, const unsigned long& righe, const unsigned long& colonne, const unsigned long& numValori)
  {
    default_random_engine gen(random_device{}());
    uniform_int_distribution <unsigned long> distRow(0,righe - 1);
    uniform_int_distribution <unsigned long> distCol(0,colonne - 1);
    ...
    mat.ColumnResize(colonne);
    int lenght;
    uniform_int_distribution<unsigned int> distDim(1,10);
    mt19937 generator{random_device{}()};
    for(unsigned int i=0; i<numValori; i++){
      r = distRow(gen);
      c = distCol(gen);
      lenght = distDim(gen);
      auto generate_len = lenght;
      string rand_str(generate_len, '\0');
      for(auto& dis: rand_str){
      dis = distChar(generator);
      }
      mat(r,c) = rand_str;
      }
}
  void MenuCSR(int tipo, int row, int column)
  {
    int scelta = -1;
    int nvalori = 0;
    lasd::MatrixCSR<int>    intMatrix;
    lasd::MatrixCSR<float>  floatMatrix;
    lasd::MatrixCSR<string> stringMatrix;

    cout << "Quanti valori vuoi inserie?" << endl;
    cin  >> nvalori;
    if(tipo == 1)
    {
      popolaMatrix(intMatrix, row, column, nvalori);
    }
    if(tipo == 2)
    {
      popolaMatrix(floatMatrix, row, column, nvalori);
    }
    if(tipo == 3)
    {
      popolaMatrixString(stringMatrix, row, column, nvalori);
    }
